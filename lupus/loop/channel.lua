...
local function remove_waiter(list, task)
  for i, t in ipairs(list) do
    if t == task then table.remove(list, i) return end
  end
end

local function wake_one(list)
  local task = table.remove(list, 1)
  if task then loop._wake(task, true) end
end
...
function Channel:send(v)
  ...
  while self.capacity and not self.closed and self:size() >= self.capacity do
    ...
    loop._park(function() remove_waiter(self.send_waiters, me) end)
  end
  if self.closed then
    return nil, "closed"
  end
...
function Channel:recv()
  while true do
    ...
    loop._park(function() remove_waiter(self.recv_waiters, me) end)
  end
end
...
function Channel:close()
  ...
  self.closed = true
  while #self.recv_waiters > 0 do wake_one(self.recv_waiters) end
  while #self.send_waiters > 0 do wake_one(self.send_waiters) end
end
