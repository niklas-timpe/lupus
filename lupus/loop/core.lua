-- Scheduler core: tasks, park/wake, timers, readers, and the run loop
-- itself. Split out of loop/init.lua so that lupus.loop.process and
-- lupus.loop.channel can depend on these primitives directly instead of
-- requiring the whole lupus.loop module back — that mutual dependency
-- (loop -> process -> loop) is exactly the kind of cycle Lua's require()
-- can't unwind, since a require during load only ever sees whatever fields
-- were assigned before the cycle was entered.
--
-- lupus/loop/init.lua builds the public lupus.loop module on top of this:
-- it adds on_signal (which never touches scheduler state) and attaches
-- lupus.loop.process, both as plain top-level requires.
...
function loop.timer(ms, fn)
	...
	entry.handle:start(ms, 0, function()
		...
		if not ok then
			log.error("timer callback failed: %s", tostring(err))
		end
	end)
	return entry
end
...
local function reader_start(self)
	if self.reading or self.eof or self.closed then
		return
	end
	self.reading = true
