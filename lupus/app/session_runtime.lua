...
function runtime_mod.new(opts)
  ...
  elseif opts.session == "continue" then
    self.session = session_mod.continue_recent(cwd, cfg.dirs.data)
      or session_mod.create(cwd, cfg.dirs.data)
  else
  ...
  self.agent = agent_mod.Agent.new{
    ...
    api_key = function(m) return models.api_key(m, cfg.settings) end,
    hooks = {
      before_tool = function(tc, args)
        return self.hub:veto("tool_call", {
          id = tc.id, tool_name = tc.name, arguments = args,
        })
      end,
...
function Runtime:emit(ev)
  for _, fn in ipairs(self.listeners) do
    ...
    if not ok then log.error("runtime listener failed (%s): %s", ev.type, tostring(err)) end
  end
end
...
function Runtime:reresolve_model()
  local wanted = self.opts.model
    or self.session:model_id()
    or self.config.settings.default_model
...
function Runtime:set_model(model, opts)
  ...
  if opts and opts.persist then
    ...
    if self.config.settings.default_model ~= key then
      ...
      config_mod.update_global(function(s) s.default_model = key end)
    end
  end
  ...
  for _, msg in ipairs(self.agent.messages) do
    ...
    elseif type(c) == "table" then
      for _, block in ipairs(c) do
        ...
        if block.arguments then chars = chars + 64 end
      end
    end
  end
