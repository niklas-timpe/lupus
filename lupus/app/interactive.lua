...
function Interactive:header_line()
	return S.bold("lupus")
		.. S.gray(("  %s/%s  ·  %s"):format(self.runtime.model.provider, self.runtime.model.id, self.runtime.cwd))
end
...
function Interactive:run_command(cmd, args)
	loop.spawn(function()
		...
		if not ok then
			self:notify(S.red("command failed: " .. tostring(err)))
		end
	end)
end
	...
	ui.input = function(opts)
		...
		local wrapper = {
			render = function(_, width)
				...
				for _, l in ipairs(editor:render(width)) do
					lines[#lines + 1] = l
				end
				return lines
			end,
...
function Interactive:run()
	...
	self.runtime:subscribe(function(ev)
		self:on_event(ev)
	end)
		...
		for _, sc in ipairs(self.runtime.host.shortcuts) do
			if sc.key == ev.name then
				loop.spawn(function()
					...
					if not ok then
						self:notify(S.red("shortcut failed: " .. tostring(err)))
					end
				end)
				return true
			end
	...
	if not self.runtime:model_available() then
		...
		self:notify(
			S.yellow("no API key for " .. self.runtime.model.provider)
				.. S.gray(
					(" — the selected model %s/%s needs one. " .. "/login adds a key, /model switches models."):format(
						self.runtime.model.provider,
						self.runtime.model.id
					)
				)
		)
...
function Interactive:replay_message(msg)
	...
	elseif msg.role == "assistant" then
		...
		if #buf > 0 then
			self:push(Markdown.new(table.concat(buf)))
		end
		for _, block in ipairs(msg.content) do
