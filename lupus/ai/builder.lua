...
function Builder:text_delta(s)
	...
	local block = ensure_block(self, "text", function()
		return types.text_block("")
	end)
...
function Builder:thinking_delta(s)
	...
	local block = ensure_block(self, "thinking", function()
		return types.thinking_block("")
	end)
...
function Builder:tool_delta(fragment)
	local block = self:current()
	if not block or block.type ~= "tool_call" then
		return
	end
	if fragment == "" then
		return
