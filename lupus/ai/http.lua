...
function http.request(opts)
	...
	local argv = {
		...
		"-i",
		"--connect-timeout",
		tostring(opts.timeout_connect or 15),
		"-X",
		opts.method or "GET",
	}
	...
	loop.spawn(function()
		while true do
			...
			if #stderr_buf > 64 then
				table.remove(stderr_buf, 1)
			end
		end
	end)
	...
	while true do
		...
		if not head then
			...
			if msg == "" then
				msg = "connection failed (curl exit " .. code .. ")"
			end
			return nil, msg
		end
		buf = rest
		...
		if status < 100 or status >= 200 then
			break
		end
	end
