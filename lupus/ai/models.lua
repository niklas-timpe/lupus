...
local builtin = {
  ...
  { id = "claude-opus-4-5", name = "Claude Opus 4.5", provider = "anthropic",
    reasoning = true, context_window = 200000, max_tokens = 32000,
    cost = { input = 5, output = 25, cache_read = 0.5, cache_write = 6.25 } },
  { id = "claude-sonnet-4-5", name = "Claude Sonnet 4.5", provider = "anthropic",
    reasoning = true, context_window = 200000, max_tokens = 64000,
...
function models.load(config)
  ...
  local function add(entry)
    ...
    if not registry.by_key[m.id] then registry.by_key[m.id] = m end
  end

  for _, entry in ipairs(builtin) do add(entry) end
  for _, entry in ipairs(config.models or {}) do add(entry) end
  return registry
end

local function ensure_loaded()
  if not registry then models.load({}) end
end
...
function models.get(query)
  ...
  for _, m in ipairs(registry.list) do
    if m.id:lower():find(needle, 1, true)
      or m.name:lower():find(needle, 1, true) then
      return m
    end
  end
  return nil
...
function models.api_key(model, config)
  ...
  if keys[model.provider] then return keys[model.provider] end
  local prov = providers[model.provider]
  if prov and prov.api_key then return prov.api_key end
  if prov and prov.env then return os.getenv(prov.env) end
  return nil
end
...
function models.available(model, config)
  local prov = providers[model.provider]
  if prov and prov.env == nil and not prov.api_key then return true end
...
function models.default(config)
  ensure_loaded()
  for _, m in ipairs(registry.list) do
    if models.available(m, config) then return m end
  end
  return nil
end
