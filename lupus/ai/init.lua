...
local adapters = {
  ["anthropic-messages"] = function() return require("lupus.ai.providers.anthropic") end,
  ["openai-completions"] = function() return require("lupus.ai.providers.openai") end,
}

--- Register a custom wire adapter (extensions can add providers).
function ai.register_api(name, adapter)
  adapters[name] = function() return adapter end
end
