...
function schema.string(opts) return node("string", opts) end
function schema.number(opts) return node("number", opts) end
function schema.integer(opts) return node("integer", opts) end
function schema.boolean(opts) return node("boolean", opts) end
...
local function lua_typename(v)
  if json.is_null(v) then return "null" end
  local t = type(v)
  if t == "number" then
...
validate_node = function(spec, value, path)
  if value == nil or json.is_null(value) then
    if spec.default ~= nil then return true, spec.default end
  ...
  if kind == "string" then
    if type(value) ~= "string" then return false, type_error(path, "string", lua_typename(value)) end
    if spec.enum then
      for _, v in ipairs(spec.enum) do
        if v == value then return true, value end
      end
      return false, ("field '%s': expected one of [%s], got %q"):format(path, table.concat(spec.enum, ", "), value)
    end
    return true, value
  elseif kind == "integer" then
    if type(value) ~= "number" then return false, type_error(path, "integer", lua_typename(value)) end
    if value % 1 ~= 0 then return false, type_error(path, "integer", "number " .. tostring(value)) end
    return true, value
  elseif kind == "number" then
    if type(value) ~= "number" then return false, type_error(path, "number", lua_typename(value)) end
    return true, value
  elseif kind == "boolean" then
    if type(value) ~= "boolean" then return false, type_error(path, "boolean", lua_typename(value)) end
    return true, value
  elseif kind == "array" then
    if type(value) ~= "table" then return false, type_error(path, "array", lua_typename(value)) end
    local out = {}
  ...
  elseif kind == "object" then
    if type(value) ~= "table" then return false, type_error(path, "object", lua_typename(value)) end
    local out = {}
    -- Unknown keys pass through untouched (models sometimes add extras).
    for k, v in pairs(value) do
      if not json.is_null(v) then out[k] = v end
    end
...
to_json_node = function(spec)
  ...
  if spec.kind == "object" then
    ...
    for _, name in ipairs(names) do
      ...
      if field.required then required[#required + 1] = name end
    end
  ...
  elseif spec.kind == "array" then
    ...
    out.type = spec.kind
    if spec.enum then out.enum = json.array(spec.enum) end
  end
  if spec.desc then out.description = spec.desc end
  if spec.default ~= nil then out.default = spec.default end
  return out
end
