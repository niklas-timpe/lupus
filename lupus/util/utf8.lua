...
local function decode(s, i)
  ...
  local len, cp
  if b >= 0xF0 then len, cp = 4, b - 0xF0
  elseif b >= 0xE0 then len, cp = 3, b - 0xE0
  elseif b >= 0xC0 then len, cp = 2, b - 0xC0
  else return nil end
  for k = 1, len - 1 do
    local c = byte(s, i + k)
    if not is_cont(c) then return nil end
    cp = cp * 64 + (c - 0x80)
  end
  return cp, len
end
...
function utf8.char(...)
  ...
  for k = 1, select("#", ...) do
    ...
    elseif cp < 0x10000 then
      ...
      out[#out + 1] = char(0xF0 + floor(cp / 262144), 0x80 + floor(cp / 4096) % 64,
...
function utf8.offset(s, n, i)
  ...
  if n == 0 then
    while i > 1 and is_cont(byte(s, i)) do i = i - 1 end
    return i
  end
  if n > 0 then
    n = n - 1
    while n > 0 and i <= len do
      i = i + 1
      while is_cont(byte(s, i)) do i = i + 1 end
      n = n - 1
    end
    if n > 0 or i > len + 1 then return nil end
    return i
  end
