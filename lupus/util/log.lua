...
local function open_path(path)
  ...
  if dir then require("lupus.util.fs").mkdirp(dir) end
  file = io.open(path, "a")
end
...
function log.setup(opts)
  ...
  if opts.path then open_path(opts.path) end
end

local function write(level, fmt, ...)
  if not file then
    local env = os.getenv("LUPUS_LOG")
    if env and env ~= "" then open_path(env) else return end
    if not file then return end
  end
  if level_order[level] < level_order[min_level] then return end
