...
local function project_trusted(host, cwd, state_dir, report)
  ...
  if not host.ui then
    report(("skipping untrusted project extensions in %s/.lupus/extensions "
      .. "(run interactively once to trust this project)"):format(cwd))
    return false
  end
  local answer = host.ui.confirm{
    title = ("Load project extensions from %s/.lupus/extensions? "
      .. "They run with your full user permissions."):format(cwd),
  }
...
function loader.load_all(host, opts)
  ...
  if not opts.no_discovery then
    ...
    if #project_files > 0
      and project_trusted(host, runtime.cwd, cfg.dirs.state, report) then
