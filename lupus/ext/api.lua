...
function api_mod.build(host, source_path)
  ...
  function api.register_tool(def)
    assert(type(def) == "table" and def.name and def.execute and def.parameters,
  ...
  function api.register_command(def)
    assert(type(def) == "table" and def.name and def.run,
      "register_command needs { name, run }")
    def.description = def.description or ("from " .. source_path)
    host.commands:register(def)
  end

  function api.register_shortcut(def)
    assert(type(def) == "table" and def.key and def.run,
      "register_shortcut needs { key, run }")
    host.shortcuts[#host.shortcuts + 1] = def
  end

  function api.register_flag(name, default)
    if host.flags[name] == nil then host.flags[name] = default end
  end
  ...
  function api.set_status(text)
    if host.ui and host.ui.set_status then host.ui.set_status(text) end
  end
  ...
  function api.exec(argv, opts)
    ...
    if opts.timeout then
      timer = loop.timer(opts.timeout * 1000, function() proc:terminate() end)
    end
    local out, err_out = {}, {}
