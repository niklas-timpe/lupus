...
function Agent:emit(type_, extra)
	...
	for _, fn in ipairs(self.listeners) do
		...
		if not ok then
			log.error("agent listener failed (%s): %s", type_, tostring(err))
		end
	end
end
...
function Agent:run(pending)
	...
	while true do
		...
		while true do
			...
			if #tool_calls == 0 and #pending == 0 then
				break
			end
		end
...
function Agent:resolve_api_key()
	if type(self.api_key) == "function" then
...
local function error_result(tc, message)
	return types.tool_result(tc.id, tc.name, { types.text_block(message) }, true)
end
...
function Agent:execute_one(tc)
	...
	local ctx = {
		...
		tool_call_id = tc.id,
		aborted = function()
			return self.abort_flag.aborted
		end,
