...
function text.cp_width(cp)
	...
	if in_ranges(zero_ranges, cp) then
		return 0
	end
	if in_ranges(wide_ranges, cp) then
		return 2
	end
	return 1
end
...
local function ansi_end(s, i)
	...
	elseif nxt == 0x5D then -- OSC: ESC ] ... BEL | ESC \
		...
		while j <= #s do
			...
			if b == 0x1B and s:byte(j + 1) == 0x5C then
				return j + 1
			end
			j = j + 1
		end
			...
			elseif cell.ansi then
				...
				if col + cell.w > width then
					if break_at and break_at < #line then
						...
						local tail = {}
						for i = break_at + 1, #line do
							tail[#tail + 1] = line[i]
						end
						for i = #line, break_at + 1, -1 do
							line[i] = nil
						end
						-- Drop the trailing space we broke on.
						if #line > 0 and line[#line].s == " " then
							line[#line] = nil
						end
						local tail_col = 0
						for _, c in ipairs(tail) do
							tail_col = tail_col + c.w
						end
						flush(active)
						for _, c in ipairs(tail) do
							line[#line + 1] = c
...
function text.truncate(s, max, ellipsis)
	...
	if text.visible_width(s) <= max then
		return s
	end
	...
	for _, cell in ipairs(text.cells(s)) do
		if cell.ansi then
			out[#out + 1] = cell.s
			if sgr_params(cell) then
				styled = true
			end
		else
			if col + cell.w > budget then
				break
			end
			out[#out + 1] = cell.s
			col = col + cell.w
