...
function Editor:add_history(entry)
  ...
  if self.history[#self.history] == entry then return end
...
local function next_offset(line, col)
  if col >= #line then return nil end
  local i = col + 1
  local b = line:byte(i)
  local len = 1
...
local function wrap_line(line, width)
  ...
  local function flush(next_start)
    local parts = {}
    for _, c in ipairs(row_cells) do parts[#parts + 1] = c.s end
  ...
  for _, cell in ipairs(cells) do
    if col + cell.w > width and #row_cells > 0 then
      if break_cell and break_cell < #row_cells then
        ...
        local tail = {}
        for i = break_cell + 1, #row_cells do tail[#tail + 1] = row_cells[i] end
        for i = #row_cells, break_cell + 1, -1 do row_cells[i] = nil end
        local tail_bytes = 0
        for _, c in ipairs(tail) do tail_bytes = tail_bytes + #c.s end
    ...
    byte = byte + #cell.s
    col = col + cell.w
    if cell.s == " " then break_cell = #row_cells end
  end
  flush(byte)
  return rows
end
...
local function cursor_row(rows, cur)
  ...
  for i = #rows, 1, -1 do
    if rows[i].line == cur.line then return i end
  end
  return #rows
end
...
local function place_in_row(self, row, target_vcol)
  ...
  for _, cell in ipairs(text.cells(row.s)) do
    if col + cell.w > target_vcol then break end
    col = col + cell.w
    byte = byte + #cell.s
  end
...
function Editor:history_next()
  if self.history_pos == nil then return end
...
function Editor:handle_input(ev, width)
  ...
  elseif name == "up" then
    if not self:move_vertical(-1, width) then self:history_prev() end
  elseif name == "down" then
    if not self:move_vertical(1, width) then self:history_next() end
...
function Editor:render(width)
  ...
  if content_width < 4 then content_width = 4 end
  ...
  if self:is_empty() then
    lines[#lines + 1] = "> " .. CURSOR_OPEN .. " " .. text.RESET
      .. text.style.dim(self.placeholder)
    lines[#lines + 1] = border
    return lines
  end
