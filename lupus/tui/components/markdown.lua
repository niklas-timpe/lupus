...
local function style_inline(s, prefix)
  ...
  s = s:gsub("%*%*%*([^%*]+)%*%*%*", function(t) return "\27[1;3m" .. t .. text.RESET .. prefix end)
  s = s:gsub("%*%*([^%*]+)%*%*", function(t) return "\27[1m" .. t .. text.RESET .. prefix end)
  s = s:gsub("%*([^%*%s][^%*]-)%*", function(t) return "\27[3m" .. t .. text.RESET .. prefix end)
  s = s:gsub("~~([^~]+)~~", function(t) return "\27[9m" .. t .. text.RESET .. prefix end)
...
function Markdown:render(width)
  ...
  local function emit(s)
    for _, l in ipairs(text.wrap(s, width)) do out[#out + 1] = l end
  end

  for _ = 1, self.padding_y do out[#out + 1] = "" end
  ...
  for _, line in ipairs(text.split_lines(self.content)) do
    ...
    if fence then
      if in_code then
        ...
        if lang then out[#out + 1] = S.gray("│ " .. lang) end
      end
    elseif in_code then
  ...
  if in_code then flush_code() end
