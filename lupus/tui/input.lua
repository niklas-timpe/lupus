...
function input.parse(buf)
	...
	while i <= n do
			...
			elseif i == n then
				...
				elseif nxt == 0x5D then -- OSC (terminal response): swallow
					...
					while j <= n do
						...
						if c == ESC and buf:byte(j + 1) == 0x5C then
							stop = j + 1
							break
						end
						j = j + 1
		...
		elseif b < 0x20 or b == 0x7F then
			...
			if name then
				events[#events + 1] = key_event(name)
			end
			i = i + 1
		else
