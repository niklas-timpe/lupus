...
function TUI:add_key_listener(fn)
	...
	return function()
		for i, f in ipairs(self.key_listeners) do
			if f == fn then
				table.remove(self.key_listeners, i)
				return
			end
		end
	end
...
function TUI:request_render(force)
	...
	if self.render_scheduled or not self.running then
		return
	end
	...
	loop.timer(delay, function()
		...
		if not ok then
			log.error("render failed: %s", tostring(err))
		end
	end)
end
...
function TUI:render_frame()
	...
	if self.force_next then
		...
		if self.surface.invalidate then
...
function TUI:start(opts)
	...
	if self.input_reader then
		self.input_task = loop.spawn(function()
			input_mod.run_reader(self.input_reader, function(ev)
				self:dispatch(ev)
			end)
		end)
	end
...
function TUI:stop()
	...
	self.running = false
	if self.input_task then
		self.input_task:cancel()
	end
	if self.resize_unsub then
		self.resize_unsub()
	end
	if self.tty_in then
