...
function screen_mod.restore()
  if not C.isendwin() then C.endwin() end
end
...
function Screen:size()
  if not self.win then return 80, 24 end
...
function Screen:attr_for(style)
  local a = 0
  if style.bold then a = bor(a, nc.A.BOLD) end
  if style.dim then a = bor(a, nc.A.DIM) end
  if style.italic then a = bor(a, nc.A.ITALIC) end
  if style.underline then a = bor(a, nc.A.UNDERLINE) end
  if style.reverse then a = bor(a, nc.A.REVERSE) end
  local fg = style.fg
...
function Screen:invalidate()
  if self.win then C.clearok(self.win, true) end
end
