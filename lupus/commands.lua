...
function commands_mod.register_builtins(registry)
		...
		run = function(ctx, args)
			...
			if provider then
				...
				for _, p in ipairs(providers) do
					...
					local desc = has_key and "key configured" or (p.env and ("env " .. p.env) or "no key needed")
			...
			local hint = ctx.runtime.model.provider == provider and "ready to go" or "/model to switch models"
	...
	registry:register({
		...
		run = function(ctx)
			...
			ctx.notify(
				("context: ~%d tokens (%d%% of %d) · total cost: $%.4f"):format(
					usage.tokens,
					usage.percent,
					usage.window,
					ctx.runtime:total_cost()
				)
			)
		end,
	})
