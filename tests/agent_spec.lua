...
describe("agent", function()
	it("runs a simple text turn", function()
		...
		assert.same({
			...
			"message_update",
			"message_update",
			"message_update", -- text start/delta/end
			"message_end",
			"turn_end",
			"agent_end",
		}, event_types(events))
	...
	it("executes tool calls and feeds results back", function()
		local agent, events, model = run_agent({
			{
				{ text = "let me check" },
				{ tool = { id = "t1", name = "echo", args = { value = "hi" } } },
				stop = "tool_use",
			},
			{ { text = "done" }, stop = "stop" },
		})
		...
		for _, ev in ipairs(events) do
			if ev.type == "tool_start" then
				has_tool_start = true
			end
			if ev.type == "tool_end" then
				has_tool_end = true
			end
		end
