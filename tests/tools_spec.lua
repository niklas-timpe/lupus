...
local function ctx(overrides)
  local c = { cwd = tmp, tool_call_id = "t1", aborted = function() return false end }
...
describe("built-in tools", function()
  ...
  describe("bash", function()
    ...
    it("aborts when requested", function()
      local aborted = false
      local c = ctx({ aborted = function() return aborted end })
      local ok, err
      loop.run(function()
        loop.timer(100, function() aborted = true end)
  ...
  describe("truncate helper", function()
    it("keeps the tail for bash-style output", function()
      local long = {}
      for i = 1, 3000 do long[i] = "line " .. i end
