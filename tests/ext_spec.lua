...
describe("ext.events hub", function()
  it("notify runs handlers in order and isolates errors", function()
    ...
    local order = {}
    hub:on("agent_start", function() order[#order + 1] = "a" end)
    hub:on("agent_start", function() error("boom") end)
    hub:on("agent_start", function() order[#order + 1] = "b" end)
  ...
  it("veto short-circuits on the first block", function()
    local hub = events_mod.new()
    local calls = 0
    hub:on("tool_call", function() calls = calls + 1 end)
    hub:on("tool_call", function() calls = calls + 1 return { block = true, reason = "no" } end)
    hub:on("tool_call", function() calls = calls + 1 end)
  ...
  it("transform chains values and supports handled", function()
    local hub = events_mod.new()
    hub:on("user_input", function(v) return v .. "!" end)
    hub:on("user_input", function() return nil end) -- unchanged
    hub:on("user_input", function(v) return v .. "?" end)
    local out, handled = hub:transform("user_input", "hi", {})
    assert.equals("hi!?", out)
    assert.is_false(handled)

    hub:on("user_input", function() return { handled = true } end)
  ...
  it("collect gathers table results", function()
    local hub = events_mod.new()
    hub:on("before_agent_start", function() return { system_prompt_append = "A" } end)
    hub:on("before_agent_start", function() return nil end)
    hub:on("before_agent_start", function() return { inject_message = "B" } end)
  ...
  it("rejects unknown event names", function()
    local hub = events_mod.new()
    assert.has_error(function() hub:on("no_such_event", function() end) end)
  end)
end)
...
describe("ext loader + runtime integration", function()
  ...
  it("loads global extensions and fires session_start", function()
    ...
    fs.write_file(tmp .. "/cfg/lupus/extensions/probe.lua", [[
      return function(api)
  ...
  it("skips untrusted project extensions in headless mode", function()
    ...
    fs.write_file(tmp .. "/proj/.lupus/extensions/evil.lua",
      "return function(api) _G.__evil = true end")
    local rt = make_runtime()
    loop.run(function()
      local loaded = rt:load_extensions(nil)
  ...
  it("loads project extensions when trusted via the ui dialog", function()
    fs.mkdirp(tmp .. "/proj/.lupus/extensions")
    fs.write_file(tmp .. "/proj/.lupus/extensions/ok.lua",
      "return function(api) _G.__proj_loaded = true end")
    local rt = make_runtime()
    local asked = 0
    local ui = {
      notify = function() end,
  ...
  it("extension tools are callable by the agent", function()
    fs.mkdirp(tmp .. "/cfg/lupus/extensions")
    fs.write_file(tmp .. "/cfg/lupus/extensions/mytool.lua", [[
  ...
  it("tool_call veto blocks built-in tools", function()
    fs.mkdirp(tmp .. "/cfg/lupus/extensions")
    fs.write_file(tmp .. "/cfg/lupus/extensions/guard.lua", [[
      return function(api)
  ...
  it("user_input transform and before_agent_start apply", function()
    fs.mkdirp(tmp .. "/cfg/lupus/extensions")
    fs.write_file(tmp .. "/cfg/lupus/extensions/shape.lua", [[
      return function(api)
