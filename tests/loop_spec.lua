...
describe("loop", function()
  it("runs a main task to completion and returns its results", function()
    local ok, a, b = loop.run(function() return 1, "two" end)
    assert.is_true(ok)
    assert.equals(1, a)
    assert.equals("two", b)
  end)
  ...
  it("propagates main task errors as ok=false", function()
    local ok, err = loop.run(function() error("boom") end)
    assert.is_false(ok)
  ...
  it("spawns tasks and joins them", function()
    local ok, sum = loop.run(function()
      local t1 = loop.spawn(function() return 10 end)
  ...
  it("timer callbacks fire, intervals repeat, cancel stops them", function()
    local ticks = 0
    loop.run(function()
      local h = loop.interval(5, function() ticks = ticks + 1 end)
      loop.sleep(28)
      h:cancel()
...
describe("channel", function()
  it("passes values between tasks in order", function()
    ...
    loop.run(function()
      ...
      for v in ch:iter() do got[#got + 1] = v end
    end)
  ...
  it("bounded channels apply backpressure", function()
    ...
    loop.run(function()
      ...
      for _ in ch:iter() do loop.sleep(1) end
