...
local function per_million(per_token)
  if not per_token then return 0 end
...
local function map_input(modalities)
  if not modalities then return { "text" } end
  local input = {}
  for _, m in ipairs(modalities) do
    if m == "text" and not input.text then input[#input + 1] = "text" end
    if m == "image" and not input.image then input[#input + 1] = "image" end
  end
...
local VENDOR_PREFIX = "^([Aa]nthropic|[Aa]mazon|[Mm]eta|[Aa]i21|[Cc]ohere|[Mm]istral|[Mm]istralAI|[Gg]oogle|[Dd]eepseek|[Qq]wen|[Mm]icrosoft|[Oo]pen[Aa][Ii]|[Bb]ytedance|[Nn]vidia|[Ii][Bb][Mm]%-[Gg]ranite|[Mm]oonshot[Aa][Ii]|[Uu][Ss]|[Ee][Uu]|[Aa]pac|[Gg]lobal)[%.%-]"
      ...
      for model_id, model_def in pairs(provider_models) do
        ...
        if output then
          for _, v in ipairs(id_variants(model_id)) do
            if not lookup[v] then lookup[v] = output end
          end
        end
      end
    end
...
return function(api)
  ...
  api.notify(("eden-ai: loaded %d models%s"):format(
    #lupus_models,
  ...
  if not api.config.providers then api.config.providers = {} end
  api.config.providers["edenai"] = {
    base_url = BASE_URL,
    env = "EDENAI_API_KEY",
  }
  if not api.config.models then api.config.models = {} end
  ...
  api.on("message_end", function(ev)
    ...
    if msg.role ~= "assistant" then return end
    if msg.provider ~= "edenai" or msg.stop_reason ~= "error" then return end
    local err = msg.error_message or ""
    if err:find(OVERFLOW, 1, true) then return end
